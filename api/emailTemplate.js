/**
 * Template HTML SafeMine para e-mails (servidor / WhatsApp / Resend).
 * Espelha src/lib/emailTemplate.js — sem dependências do Vite.
 */
...
function formatValue(v) {
  ...
  if (Array.isArray(v)) {
    if (v.length && v[0]?.url) {
      return v
        .map((a) => `<a href="${esc(a.url)}" style="color:#FF5E14">${esc(a.name || a.url)}</a>`)
        .join('<br/>')
    }
    return esc(v.join('; '))
  }
...
function rowsFromDados(dados = {}) {
  ...
  const keys = [
    ...
    ...Object.keys(dados).filter(
      (k) => !preferred.includes(k) && !k.startsWith('_') && dados[k] != null && dados[k] !== '',
    ),
  ]
  const seen = new Set()
...
/**
 * @param {{ tipo: string, dados: object, userEmail?: string, createdAt?: string, numero?: string, canal?: string }} opts
 */
export function buildReportEmailHtml({
  ...
  const canalLabel =
    canal === 'whatsapp' || dados?._canal === 'whatsapp'
      ? 'WhatsApp'
      : canal === 'radio' || dados?._canal === 'radio'
        ? 'Rádio'
        : 'App'
  const numeroLabel = numero || dados?._numero || ''
        ...
        <table role="presentation" width="100%" style="max-width:640px;background:#ffffff;border-radius:16px;overflow:hidden;border:1px solid #EDF0F3;box-shadow:0 8px 28px rgba(74,85,104,0.08);">
          <tr>
            <td style="background:linear-gradient(135deg,#FF7A33,#FF5E14);padding:22px 24px;">
              ...
              <div style="color:#FFE8D6;font-size:14px;margin-top:6px;">${esc(tipoLabel)}${numeroLabel ? ` · ${esc(numeroLabel)}` : ''}</div>
            </td>
          </tr>
          <tr>
              ...
              <table width="100%" cellspacing="0" cellpadding="0" style="background:#FFF4EC;border-radius:12px;border:1px solid #FFE4D0;">
                <tr>
                  <td style="padding:14px 16px;">
                    ...
                    <div style="font-size:12px;color:#8B939E;margin-top:10px;">Canal</div>
                    <div style="font-size:14px;color:#4A5568;font-weight:600;">${esc(canalLabel)}</div>
                    ${
                      numeroLabel
                        ? `<div style="font-size:12px;color:#8B939E;margin-top:10px;">Nº do relato</div>
                    <div style="font-size:14px;color:#4A5568;font-weight:600;">${esc(numeroLabel)}</div>`
                        : ''
                    }
                    ${
                      userEmail
                        ? `<div style="font-size:12px;color:#8B939E;margin-top:10px;">Usuário do sistema</div>
                    <div style="font-size:14px;color:#4A5568;font-weight:600;">${esc(userEmail)}</div>`
                        : ''
                    }
                    ${
                      dados?._wa_phone
                        ? `<div style="font-size:12px;color:#8B939E;margin-top:10px;">WhatsApp</div>
                    <div style="font-size:14px;color:#4A5568;font-weight:600;">${esc(dados._wa_phone)}</div>`
                        : ''
                    }
                  </td>
                </tr>
          ...
          ${
            ...
            <td style="padding:8px 24px 8px;">
              <div style="font-size:12px;font-weight:800;letter-spacing:.6px;text-transform:uppercase;color:#8B939E;margin:8px 0 10px;">Transcrição do áudio</div>
              <div style="background:#F7F8FA;border:1px solid #EDF0F3;border-radius:12px;padding:14px 16px;color:#4A5568;font-size:13px;line-height:1.55;">${esc(dados._transcript).replace(/\n/g, '<br/>')}</div>
            </td>
          </tr>`
              : ''
          }
