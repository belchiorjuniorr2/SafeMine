...
export default function SafetyInspection() {
  ...
  const handleSubmit = async () => {
    setSubmitting(true)
    await supabase.from('registros').insert({ tipo: 'inspecao', dados: f, user_id: user.id, user_email: user.email })
    setSubmitting(false)
    navigate('/sucesso', { state: { type: 'inspecao', data: f } })
  }
      ...
      <div style={{ padding: '16px' }}>
          ...
          <div style={{ marginBottom: '16px' }}>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              {tiposInspecao.map(t => (
                <button
                  key={t}
                  onClick={() => setF(p => ({ ...p, tipo_inspecao: t }))}
                  style={{
                    flex: 1, padding: '10px 0', borderRadius: '10px',
                    border: `2px solid ${f.tipo_inspecao === t ? 'var(--orange)' : 'var(--gray-mid)'}`,
                    background: f.tipo_inspecao === t ? 'rgba(255,94,20,0.1)' : '#fff',
                    color: f.tipo_inspecao === t ? 'var(--orange)' : 'var(--gray)',
        ...
        <div style={{ background: '#fff', borderRadius: '16px', padding: '16px', boxShadow: 'var(--shadow)', marginBottom: '12px' }}>
          ...
          {[['Conformidades', 'conformidades', 'Itens em conformidade...'],
            ...
            ['Recomendações', 'recomendacoes', 'Ações recomendadas...']].map(([label, key, ph]) => (
            <div key={key} style={{ marginBottom: '16px' }}>
              <label style={labelStyle}>{label}</label>
