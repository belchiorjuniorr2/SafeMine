...
export default function ShiftChange() {
  ...
  const handleSubmit = async () => {
    ...
    await supabase.from('registros').insert({ tipo: 'turno', dados, user_id: user.id, user_email: user.email })
    setSubmitting(false)
    navigate('/sucesso', { state: { type: 'turno', data: dados } })
  }
  ...
  const TurnoSelect = ({ label, field: fkey }) => (
    <div style={{ marginBottom: '16px' }}>
      ...
      <div style={{ display: 'flex', gap: '8px' }}>
        {turnos.map(t => (
          <button key={t} onClick={() => setF(p => ({ ...p, [fkey]: t }))} style={{ flex: 1, padding: '10px 0', borderRadius: '10px', border: `2px solid ${f[fkey] === t ? 'var(--orange)' : 'var(--gray-mid)'}`, background: f[fkey] === t ? 'rgba(255,94,20,0.1)' : '#fff', color: f[fkey] === t ? 'var(--orange)' : 'var(--gray)', fontWeight: 700, fontSize: '12px', transition: 'all 0.15s' }}>{t}</button>
        ))}
      </div>
    </div>
  )
    ...
    <div style={{ minHeight: '100vh', background: 'var(--gray-light)', paddingBottom: '32px' }}>
      ...
      <div style={{ padding: '16px' }}>
        ...
        <div style={{ background: '#fff', borderRadius: '16px', padding: '16px', boxShadow: 'var(--shadow)', marginBottom: '12px' }}>
          <div style={{ fontSize: '13px', fontWeight: 700, color: 'var(--text-dark)', marginBottom: '16px', paddingBottom: '12px', borderBottom: '1px solid var(--gray-light)' }}>Informações Gerais</div>
        ...
        <div style={{ background: '#fff', borderRadius: '16px', padding: '16px', boxShadow: 'var(--shadow)', marginBottom: '12px' }}>
          <div style={{ fontSize: '13px', fontWeight: 700, color: 'var(--text-dark)', marginBottom: '16px', paddingBottom: '12px', borderBottom: '1px solid var(--gray-light)' }}>Turno Saindo</div>
        ...
        <div style={{ background: '#fff', borderRadius: '16px', padding: '16px', boxShadow: 'var(--shadow)', marginBottom: '12px' }}>
          <div style={{ fontSize: '13px', fontWeight: 700, color: 'var(--text-dark)', marginBottom: '16px', paddingBottom: '12px', borderBottom: '1px solid var(--gray-light)' }}>Turno Entrando</div>
        ...
        <div style={{ background: '#fff', borderRadius: '16px', padding: '16px', boxShadow: 'var(--shadow)', marginBottom: '12px' }}>
          <div style={{ fontSize: '13px', fontWeight: 700, color: 'var(--text-dark)', marginBottom: '16px', paddingBottom: '12px', borderBottom: '1px solid var(--gray-light)' }}>Status Operacional</div>
