...
const inputStyle = { width: '100%', padding: '12px 14px', borderRadius: '10px', border: '1.5px solid var(--gray-mid)', fontSize: '14px', color: 'var(--text-dark)', background: '#fff', outline: 'none' }
...
export default function VehicleChecklist() {
  ...
  const handleSubmit = async () => {
    setSubmitting(true)
    await supabase.from('registros').insert({ tipo: 'veiculo', dados: f, user_id: user.id, user_email: user.email })
    setSubmitting(false)
    navigate('/sucesso', { state: { type: 'veiculo', data: f } })
  }
                ...
                {['OK', 'NOK', 'NA'].map(s => (
                  <button
                    ...
                    style={{
                      padding: '6px 12px',
                      borderRadius: '8px',
                      border: `2px solid ${f[key] === s ? statusColors[s] : 'var(--gray-mid)'}`,
                      background: f[key] === s ? `${statusColors[s]}18` : '#fff',
                      color: f[key] === s ? statusColors[s] : 'var(--gray)',
                      fontWeight: 700,
                      fontSize: '12px',
